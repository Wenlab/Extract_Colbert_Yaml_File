tic
%目的: 将rawdata中一个文件夹下所有*.yaml文件提取出name,头尾位置, angle_data,curve_data
%然后将其导入到data对应文件夹下面


TypePathname={'N2','Forward','Backward','Tosif_data'};  % 不同种类的文件夹
workpath = 'path/to/your/folder';  % 
...
for filepath_num=1:length(TypePathname)
    ...
    for s_yaml=1:length(yamlfiles)
        ...
        [~,wormdata.Speed]=relativePositionandSpeed(wormdata.Centerline,wormdata.StagePosition,wormdata.TimeElapsed); % Vx,Vy  um/s
        
        save(fullfile(savefolder,strcat(wormname,'_',savename)),'wormdata')
