...
class YamlFrames_Only_Centerline(object):
    def __init__(self, name: str, framessize: int):
        import numpy as np
        self.worm_length = 0
        self.name = name
...
def Just_Get_Centerline(yaml_path):
    ...
    All_Frames_num = end_num - begin_num1 + 1  # 总的帧数
    wormname = GetWormName(yaml_path)
    YamlFiles = YamlFrames_Only_Centerline(wormname, All_Frames_num)
    YamlFiles.ExperimentTime = Get_ExperimentTime(ListFile)
    print(YamlFiles.ExperimentTime)
...
class FileNamesExtract(object):
    def __init__(self, folderPath: str, videoType: str = '.avi'):
        super(FileNamesExtract, self).__init__()
        self.folder, self.videofiles = GetFolderFiles(folderPath, videoType)
        _, self.yamlfiles = GetFolderFiles(folderPath, '.yaml')
        self.wormnames = listdeltype(self.videofiles)


def delfiletype(filenames: str):
    return filenames.split('.')[0]


def listdeltype(files: list):
    a = []
    for file in files:
        a.append(delfiletype(file))
    return a


def GetFolderFiles(path: str, fileType: str = ".*"):
    """
    返回指定路径下指定类型的文件
    :param path:   指定路径下的文件
    :param fileType:
    ...
    if os.path.isdir(path):
        ...
        for s in filelist:
            if os.path.splitext(s)[1] == fileType:  # 指定类型的文件
                addlist.append(s)
    return path, addlist


def YamlExportMatAll(YamlDataFolder: str, SaveFolder: str = None,
                     BoundaryA=False,
                     BoundaryB=False,
                     Centerline=True,
                     DLPisOn=False,
                     DefaultGridSizeForNonProtocolIllum=False,
                     ExperimentTime=False,
                     FirstLaser=False,
                     ...
                     IllumFlipLR=False,
                     IllumInvert=False,
                     IllumRectOrigin=False,
                     IllumRectRadius=False,
                     ProtocolIsOn=False,
                     ProtocolStep=False,
                     SecondLaser=False,
                     StageFeedbackTarget=False,
                     StagePosition=False,
                     StageVelocity=False,
                     Tail=False,
                     TimeElapsed=True,
    ...
    filenames = FileNamesExtract(YamlDataFolder)

    allnum = len(filenames.wormnames)
    for i in range(0, allnum):
        ...
        YamlExportMatOne(yamlfilepath, SaveFolder, wormname + '.mat',
                         ...
                         DLPisOn=DLPisOn,
                         DefaultGridSizeForNonProtocolIllum=DefaultGridSizeForNonProtocolIllum,
                         ExperimentTime=ExperimentTime,
                         FirstLaser=FirstLaser,
                         FloodLightIsOn=FloodLightIsOn,
                         FrameNumber=FrameNumber,
                         Head=Head,
                         IllumFlipLR=IllumFlipLR,
                         IllumInvert=IllumInvert,
                         IllumRectOrigin=IllumRectOrigin,
                         IllumRectRadius=IllumRectRadius,
                         ProtocolIsOn=ProtocolIsOn,
                         ProtocolStep=ProtocolStep,
                         SecondLaser=SecondLaser,
                         StageFeedbackTarget=StageFeedbackTarget,
                         ...
                         curvedatafiltered=curvedatafiltered,
                         mean_worm_length=mean_worm_length,
                         name=name,
    ...
    print('All time cost {}'.format(yaml_time_end - yaml_time_start), end='')
...
def YamlExportMatOne(YamlPath: str,
    ...
    print("file {} Begin to Extract".format(SaveName))

    yaml_time_start = time.time()

    YamlData = Extract_Yaml_Multiprocess_one(YamlPath)

    yaml_time_end = time.time()
    print('file{} cost {} s:'.format(SaveName, yaml_time_end - yaml_time_start), end='')

    data = dict()
    if BoundaryA:
        data['BoundaryA'] = YamlData.BoundaryA
    if BoundaryB:
        data['BoundaryB'] = YamlData.BoundaryB
    if Centerline:
        data['Centerline'] = YamlData.Centerline
    if DLPisOn:
        data['DLPisOn'] = YamlData.DLPisOn
    if DefaultGridSizeForNonProtocolIllum:
        data['DefaultGridSizeForNonProtocolIllum'] = YamlData.DefaultGridSizeForNonProtocolIllum
    if ExperimentTime:
        data['ExperimentTime'] = YamlData.ExperimentTime
    if FirstLaser:
        data['FirstLaser'] = YamlData.FirstLaser
    if FloodLightIsOn:
        data['FloodLightIsOn'] = YamlData.FloodLightIsOn
    if FrameNumber:
        data['FrameNumber'] = YamlData.FrameNumber
    if Head:
        data['Head'] = YamlData.Head
    if IllumFlipLR:
        data['IllumFlipLR'] = YamlData.IllumFlipLR
    if IllumInvert:
        data['IllumInvert'] = YamlData.IllumInvert
    if IllumRectOrigin:
        data['IllumRectOrigin'] = YamlData.IllumRectOrigin
    if IllumRectRadius:
        data['IllumRectRadius'] = YamlData.IllumRectRadius
    if ProtocolIsOn:
        data['ProtocolIsOn'] = YamlData.ProtocolIsOn
    if ProtocolStep:
        data['ProtocolStep'] = YamlData.ProtocolStep
    if SecondLaser:
        data['SecondLaser'] = YamlData.SecondLaser
    if StageFeedbackTarget:
        data['StageFeedbackTarget'] = YamlData.StageFeedbackTarget
    if StagePosition:
        data['StagePosition'] = YamlData.StagePosition
    if StageVelocity:
        data['StageVelocity'] = YamlData.StageVelocity
    if Tail:
        data['Tail'] = YamlData.Tail
    if TimeElapsed:
        data['TimeElapsed'] = YamlData.TimeElapsed
    if angle_data:
        data['angle_data'] = YamlData.angle_data
    if curve_data:
        data['curve_data'] = YamlData.curve_data
    if curvedatafiltered:
        data['curvedatafiltered'] = YamlData.curvedatafiltered
    if mean_worm_length:
        data['mean_worm_length'] = YamlData.mean_worm_length
    if name:
        data['name'] = YamlData.name
    if worm_length:
        data['worm_length'] = YamlData.worm_length
    if savevideo:
        """
        如果要保存视频文件
        """
